...
# }
  ...
  describe 'exists?' do
    describe 'when the replicaset is not created' do
      it 'returns false' do
        provider.stubs(:_mongo).returns(<<EOT)
{
	"startupStatus" : 3,
...
EOT
    ...
    describe 'when at least one member is not running with --replSet' do
      it 'raises an error' do
        provider.stubs(:_mongo).returns('{ "ok" : 0, "errmsg" : "not running with --replSet" }')
    ...
    describe 'when no member is available' do
      it 'raises an error' do
        provider.stubs(:mongo).raises(Puppet::ExecutionFailure, <<EOT)
