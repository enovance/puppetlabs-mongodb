...
Puppet::Type.type(:mongodb_shard).provide(:mongo) do
  ...
  desc "Manage hosts members for a shard."
  ...
  def flush
    set_members
    @property_hash = self.class.get_sharding_properties
  end
  ...
  def sh_status(host)
    mongo_command("sh.status()", host)
  end

  def sh_add(host, master)
    members.each do |member|
      mongo_command("sh.add(\"#{name}/#{member}\")", master)
    end
  end

  def sh_remove(host, master)
    members.each do |member|
      mongo_command("sh.remove(\"#{name}/#{member}\")", master)
    end
  end
  ...
  def self.get_sharding_properties
    output = mongo_command('sh.conf()')
    if output['members']
